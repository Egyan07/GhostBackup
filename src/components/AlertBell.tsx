...
const ALERT_LEVEL_CONFIG: Record<string, { color: string; bg: string; icon: string }> = {
...
export default function AlertBell() {
  ...
  const topAlert = alerts.find(
    (a) => !a.dismissed && (a.level === "critical" || a.level === "error")
  );
        ...
        >
          ...
          >
            <div style={{ fontWeight: 700, fontSize: 13, color: "var(--text-primary)" }}>
              Alerts{" "}
              {unread > 0 && (
                <span style={{ color: "var(--accent-hover)", fontSize: 11 }}>
                  · {unread} unread
                </span>
              )}
            </div>
