...
export default function App() {
  ...
  useEffect(() => {
    window.ghostbackup?.version?.().then((v) => {
      if (v) setAppVersion("v" + v);
    });
    window.ghostbackup?.author?.().then((a) => {
      if (a) setAppAuthor(a);
    });
  }, []);
            ...
            <div key={sec}>
              ...
              {NAV.filter((n) => n.sec === sec).map((n) => (
                <div
                  ...
                  tabIndex={0}
                  onKeyDown={(e) => {
                    if (e.key === "Enter" || e.key === " ") setScreen(n.id);
                  }}
                >
      ...
      <div className="main">
        <div className="topbar">
          ...
          <div className="topbar-meta">
            ...
            <div className="meta-badge">Next run: {formatNextRun(health)}</div>
            <button
