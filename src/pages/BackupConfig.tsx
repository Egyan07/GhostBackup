  ...
  return (
    <div
      style={{
        ...
        border: `1px solid ${isOk ? "var(--border-subtle)" : "rgba(248,113,113,0.25)"}`,
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div
          style={{
            fontSize: 13,
            fontWeight: 600,
            color: isOk ? "var(--text-primary)" : "var(--red)",
            marginBottom: 6,
          }}
        >
        ...
        {isOk && (
          <>
            <div className="prog-track" style={{ marginBottom: 5 }}>
              <div
                className={`prog-fill ${pct > 85 ? "" : "success"}`}
                style={{
                  width: `${pct}%`,
                  background: pct > 85 ? "var(--red)" : pct > 65 ? "var(--amber)" : undefined,
                }}
              />
            </div>
            <div className="text-xs text-tertiary">
              {status.used_gb?.toFixed(1)} GB used ·{" "}
              <span style={{ color: "var(--green)" }}>{free} GB free</span> ·{" "}
              {status.total_gb?.toFixed(0)} GB total
            </div>
          </>
        )}
        {!isOk && (
          <div className="text-xs text-red">
            {status.error || "Drive not found or inaccessible"}
          </div>
        )}
      </div>
      {isOk && (
        <div style={{ flexShrink: 0, textAlign: "right" }}>
          <div
            style={{
              fontSize: 20,
              fontWeight: 800,
              color: pct > 85 ? "var(--red)" : "var(--text-primary)",
              letterSpacing: -1,
            }}
          >
            {pct.toFixed(0)}%
        ...
        c
          ? {
              ...
              sources: (c.sources || []).map((s) =>
                s.label === label || s.name === label ? source : s
              ),
            }
          : c
      );
    ...
    setCfg((c) =>
      c
        ? {
            ...
            backup: {
              ...c.backup,
              exclude_patterns: [...(c.backup?.exclude_patterns || []), newEx.trim()],
            },
          }
        : c
    );
    ...
    setCfg((c) =>
      c
        ? {
            ...
            backup: {
              ...c.backup,
              exclude_patterns: (c.backup?.exclude_patterns || []).filter((e) => e !== p),
            },
          }
        : c
    );
    ...
    <div>
      ...
      <div className="card mb-16">
        ...
        <div className="flex gap-10 items-center mb-12">
          ...
          />
          <button className="btn btn-secondary btn-sm" onClick={pickSsd}>
            Browse…
          </button>
        ...
        <div className="card">
          ...
          <div className="fg">
            ...
            <input
              ...
              onChange={(e) =>
                setCfg((c) => (c ? { ...c, schedule: { ...c.schedule, time: e.target.value } } : c))
              }
            />
          </div>
          <div className="fg">
            ...
            <select
              className="fi"
              value={cfg.schedule?.timezone || "Europe/London"}
              onChange={(e) =>
                setCfg((c) =>
                  c ? { ...c, schedule: { ...c.schedule, timezone: e.target.value } } : c
                )
              }
            >
        ...
        <div className="card">
          ...
          <div className="fg">
            <label className="input-label">
              Copy threads —{" "}
              <span className="text-accent">{cfg.performance?.concurrency || 4}</span>
            </label>
            <input
              className="fi"
              type="range"
              ...
              onChange={(e) =>
                setCfg((c) =>
                  c ? { ...c, performance: { ...c.performance, concurrency: +e.target.value } } : c
                )
              }
            />
          </div>
          <div className="fg">
            ...
            <input
              ...
              onChange={(e) =>
                setCfg((c) =>
                  c
                    ? { ...c, performance: { ...c.performance, max_file_size_gb: +e.target.value } }
                    : c
                )
              }
            />
          ...
          <div className="flex items-center gap-12">
            <label className="toggle">
              <input
                ...
                onChange={(e) =>
                  setCfg((c) =>
                    c ? { ...c, backup: { ...c.backup, verify_checksums: e.target.checked } } : c
                  )
                }
              />
            ...
            <div>
              ...
              <div className="text-xs text-tertiary">
                Slower but detects corruption. Recommended.
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="card mb-16">
        ...
        {sources.length === 0 ? (
          <div className="empty" style={{ padding: "24px 0" }}>
            <div className="empty-icon">📁</div>No source folders added yet. Add a folder to start
            backing up.
          </div>
        ) : (
          sources.map((s) => (
            <div
              className="site-row"
              key={s.label || s.name}
              style={{ opacity: s.enabled !== false ? 1 : 0.5 }}
            >
              <label className="toggle">
                <input
                  type="checkbox"
              ...
              <button
                className="btn btn-ghost btn-sm"
                onClick={() => removeFolder(s.label || s.name || "")}
      ...
      <div className="card mb-16">
        ...
        <div className="flex gap-8">
          <input
            className="fi flex-1 mono"
            placeholder="e.g.  ~$*  or  *.tmp  or  Thumbs.db"
            value={newEx}
            onChange={(e) => setNewEx(e.target.value)}
            onKeyDown={(e) => e.key === "Enter" && addEx()}
          />
          <button className="btn btn-secondary btn-sm" onClick={addEx}>
            Add
          </button>
        </div>
        <div className="tag-list">
          {(cfg.backup?.exclude_patterns || []).map((p) => (
            <span className="tag" key={p}>
              {p}
              <span className="tag-x" onClick={() => remEx(p)}>
                ×
              </span>
            </span>
          ))}
