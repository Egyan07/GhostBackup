...
function KeyRotationModal({ newKey, onConfirm, onCancel }: { newKey: string; onConfirm: () => void; onCancel: () => void }) {
    ...
    <div className="flex flex-col gap-16">
          ...
          <div>
            <div className="grid-3 mb-16" style={{ gap: 10 }}>
              {[
                ...
                { label: "Status",      value: <span className="pill pill-success">Mounted</span>, mono: false },
                ...
                { label: "Free",        value: `${ssdStatus.available_gb?.toFixed(1)} GB`,
                  color: (ssdStatus.available_gb ?? 0) < 10 ? "var(--red)" : "var(--green)", mono: false },
              ].map(s => (
        ...
        <div className="flex items-center gap-12 mb-16">
          ...
          {watcherStatus?.running ? (
            <button className="btn btn-danger btn-sm" onClick={async () => {
              ...
              catch (e) { setWatcherMsg(`✕ ${(e as Error).message}`); }
            }}>Stop Watcher</button>
          ) : (
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
            {watcherStatus.sources.map(s => (
              <div key={s.label} style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 12px", background: "var(--bg-raised)", borderRadius: "var(--r-md)", border: "1px solid var(--border-subtle)", fontSize: 12 }}>
                ...
                {(s.pending_changes ?? 0) > 0 && <span style={{ color: "var(--amber)", fontWeight: 600 }}>⏳ {s.pending_changes} pending</span>}
      ...
      <div className="card">
        ...
        <div className="grid-2">
          {[{ label: "SMTP Host", key: "host" as const }, { label: "SMTP Port", key: "port" as const, type: "number" as const }, { label: "From Address", key: "user" as const }].map(f => (
            <div className="fg" key={f.key}>
              <label className="input-label">{f.label}</label>
              <input className="fi" type={f.type || "text"} value={cfg?.smtp?.[f.key] ?? ""}
                onChange={e => setCfg(c => c ? ({ ...c, smtp: { ...c.smtp, [f.key]: f.type === "number" ? +e.target.value : e.target.value } }) : c)} />
            </div>
          ))}
          <div className="fg">
            ...
            <input className="fi"
              ...
              onChange={e => setCfg(c => c ? ({ ...c, smtp: { ...c.smtp, recipients: e.target.value.split(",").map(r => r.trim()).filter(Boolean) } }) : c)} />
          </div>
        </div>
        <div className="flex gap-10 items-center">
          ...
          <button className="btn btn-ghost btn-sm"
            onClick={async () => { setTestMsg(null); try { await api.testSmtp(); setTestMsg("✓ Test email sent successfully"); } catch (e) { setTestMsg(`✕ ${(e as Error).message}`); } }}>
            Send Test Email
          </button>
      ...
      <div className="card">
          ...
          {[
            ...
            <div className="fg" style={{ marginBottom: 0 }} key={f.k}>
              ...
              <input className="fi" type="number" min={f.min} max={f.max}
                ...
                onChange={e => setCfg(c => c ? ({ ...c, retention: { ...c.retention, [f.k]: +e.target.value } }) : c)} />
            </div>
          ))}
        </div>
        ...
        <div className="flex gap-10 items-center">
          ...
          <button className="btn btn-danger btn-sm"
            onClick={async () => { setPruneMsg(null); try { await api.runPrune(); setPruneMsg("✓ Prune job started"); } catch (e) { setPruneMsg(`✕ ${(e as Error).message}`); } }}>
            Run Prune Now
          </button>
          ...
          <button
            ...
            onClick={async () => {
              ...
              try {
                ...
                setPendingKey(key);
              } catch (e) { setKeyMsg(`✕ ${(e as Error).message}`); }
            }}
          >
            Generate New Key
          </button>
