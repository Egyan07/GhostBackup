...
import SsdGauge from "../components/SsdGauge";
...
export default function Dashboard() {
  ...
  const storage = data?.ssd_storage || { status: "unknown", used_gb: 0, total_gb: 100 };
    ...
    <div>
        ...
        <div className="alert alert-info mb-16 flex items-center gap-12">
          ...
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 600, marginBottom: 6 }}>
              Backup in progress · {active.overall_pct?.toFixed(0)}%
            </div>
            <div className="prog-track">
              <div className="prog-fill" style={{ width: `${active.overall_pct || 0}%` }} />
            </div>
          </div>
        ...
        {[
          {
            ...
            value: last?.files_transferred ?? "—",
            sub: last?.started_at?.slice(0, 10) || "No runs yet",
            icon: "📁",
          },
          {
            label: "Data Transferred",
            value: last?.bytes_human ?? "—",
            sub: "Last backup",
            icon: "📦",
          },
          {
            ...
            value: runs.length
              ...
              : "—",
            sub: `${runs.filter((r) => r.status === "success").length} / ${runs.length} runs`,
            icon: "✅",
          },
          {
            label: "Last Run Duration",
            value: last?.duration_human ?? "—",
            sub: last ? <StatusPill status={last.status} /> : "No data",
            icon: "⏱",
          },
        ].map((s, i) => (
      ...
      <div className="grid-2-1 mb-20">
        <div className="card">
          ...
          {runs.length > 0 ? (
            ...
            <div className="empty">
              <div className="empty-icon">📅</div>No backup runs yet
            </div>
          )}
          ...
          <div className="flex gap-16">
            {[
              {
                label: "Last run",
                value: last?.started_at?.slice(0, 16).replace("T", " ") || "Never",
              },
          ...
          >
            ...
            >
              ...
              {storage.status === "disconnected" && (
                <span className="pill pill-failed" style={{ marginLeft: "auto", fontSize: 10 }}>
                  Disconnected
                </span>
              )}
              {storage.status === "ok" && (
                <span className="pill pill-success" style={{ marginLeft: "auto", fontSize: 10 }}>
                  Mounted
                </span>
              )}
            </div>
            ...
            <div className="text-xs text-tertiary">
              {storage.available_gb?.toFixed(1) || "—"} GB free
            </div>
            {storage.path && (
              <div
                className="text-xs mono text-tertiary"
                style={{
                  maxWidth: "100%",
                  overflow: "hidden",
      ...
      <div className="card">
        ...
        {Object.keys(libs).length === 0 ? (
          <div className="empty">
            <div className="empty-icon">📂</div>Run a backup to see folder status
          </div>
        ) : (
            ...
            <tbody>
              {Object.entries(libs).map(([name, lib]) => (
                <tr key={name}>
                  ...
                  <td
                    className="mono"
                    style={{ color: lib.files_failed ? "var(--red)" : "var(--text-tertiary)" }}
                  >
                    {lib.files_failed || 0}
                  </td>
                </tr>
