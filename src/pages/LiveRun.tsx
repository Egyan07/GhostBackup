...
export default function LiveRun() {
  ...
  const elapsed = (() => {
    ...
    const s = Math.floor((Date.now() - new Date(utcStr).getTime()) / 1000);
    if (s < 0) return null;
    ...
    <div>
          ...
          <div style={{ textAlign: "right" }}>
            ...
            {isRunning && (
              <div className="text-xs text-tertiary mt-4 mono">
                {status.files_transferred} transferred ·{" "}
                {status.files_failed > 0 ? (
                  <span className="text-red">{status.files_failed} errors</span>
                ) : (
                  "0 errors"
                )}
              </div>
      ...
      <div className="grid-2 mb-16">
        <div className="card">
          ...
          {Object.keys(libs).length === 0 ? (
            <div className="empty" style={{ padding: "24px 0" }}>
              <div className="empty-icon">📁</div>Start a backup run
            </div>
          ) : (
            Object.entries(libs).map(([name, lib]) => (
              <div key={name} style={{ marginBottom: 16 }}>
                ...
                <div className="prog-track">
                  <div
                    className={`prog-fill ${lib.pct === 100 ? "success" : ""}`}
                    style={{ width: `${lib.pct || 0}%` }}
                  />
                </div>
              </div>
            ))
        ...
        <div className="card">
          ...
          {[
            ...
            {
              label: "Data Written",
              value: status?.bytes_transferred
                ? `${(status.bytes_transferred / 1024 ** 3).toFixed(2)} GB`
                : "—",
            },
      ...
      <div className="card">
        ...
        <div className="scroll-panel">
          {feed.length === 0 ? (
            <div className="empty" style={{ padding: "24px 0" }}>
              <div className="empty-icon">📂</div>Awaiting backup activity…
            </div>
          ) : (
            feed.map((f, i) => (
