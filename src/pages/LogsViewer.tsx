...
export default function LogsViewer() {
  ...
  const [rd, setRd] = useState<RunSummary | null>(null);
  ...
  const exportCsv = () => {
    ...
    const rows = [
      ["Time", "Level", "Message"],
      ...filtered.map((l) => [l.logged_at, l.level, l.message]),
    ];
    ...
    <div className="grid-2-1" style={{ gap: 16, alignItems: "start" }}>
        ...
        <div className="card mb-16">
          <div className="flex items-center gap-10 mb-12" style={{ flexWrap: "wrap" }}>
            ...
            <div className="flex gap-6">
              ...
              <button className="btn btn-secondary btn-sm" onClick={exportCsv} disabled={!sel}>
                Export
              </button>
            </div>
          </div>
          ...
          <div className="scroll-panel">
            {!sel ? (
              <div className="empty" style={{ padding: "24px 0" }}>
                <div className="empty-icon">📋</div>Select a run from the right panel
              </div>
            ) : filtered.length === 0 ? (
              <div className="empty" style={{ padding: "24px 0" }}>
                <div className="empty-icon">🔍</div>No logs found
              </div>
            ) : (
              <table className="table">
                ...
                <tbody>
                  {filtered.map((l, i) => (
                    <tr key={i}>
                      <td className="mono text-tertiary" style={{ fontSize: 11 }}>
                        {l.logged_at?.slice(11, 19)}
                      </td>
                      <td
                        style={{
                          color: lvlColor[l.level] || "var(--text-secondary)",
                          fontWeight: 600,
                          fontSize: 11,
                        }}
                      >
        ...
        {rd && (
          <div className="card">
            ...
            <div className="grid-3" style={{ gap: 10 }}>
              {[
                { k: "Transferred", v: rd.files_transferred },
                { k: "Failed", v: rd.files_failed },
                { k: "Duration", v: rd.duration_human },
                { k: "Data", v: rd.bytes_human },
                { k: "Status", v: <StatusPill status={rd.status} /> },
                { k: "Run ID", v: `#${rd.id}` },
              ].map((s) => (
                <div
                  key={s.k}
      ...
      <div className="card" style={{ position: "sticky", top: 0 }}>
        ...
        {loading ? (
          ...
          <div className="empty" style={{ padding: "24px 0" }}>
            <div className="empty-icon">📅</div>No runs yet
          </div>
        ) : (
          <div className="scroll-panel">
            {runs.map((r) => (
              ...
              >
                ...
                <div className="text-xs text-tertiary mono">
                  {r.started_at?.slice(0, 16).replace("T", " ")} · {r.duration_human}
                </div>
                <div className="text-xs text-tertiary mt-4">
                  {r.files_transferred} files transferred
                </div>
              </div>
            ))}
          </div>
