...
Fixes applied:
  - _dispatch() checks loop is not closed before dispatching [FIX-P3]
  - update_loop() method added so api.py can refresh the ref  [FIX-P3]
  - Graceful fallback logged when loop is unavailable         [FIX-P3]
"""
...
class FileWatcher:
    ...
    def update_loop(self, loop: asyncio.AbstractEventLoop) -> None:
        """FIX-P3: Allow api.py to refresh the event loop reference."""
    ...
    def _dispatch(self, label: str) -> None:
        """
        FIX-P3: Check loop is available and not closed before dispatching.
        Logs clearly if dispatch fails rather than silently dropping.
        """
        with self._loop_lock:
            loop = self._loop
